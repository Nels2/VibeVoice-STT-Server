...
#flashattn2 is needed!!!
```

## Usages
### Usage 1: Launch a local server (bare-bones)
This will load a bare-bones server that supports models, translations, and transcriptions endpoints. More can be added easily!
```bash
apt update && apt install ffmpeg -y # for demo
export VIBEVOICE_HF_CACHE="<Path-to-huggingface-hub-folder>"
python demo/server.py
```

### Usage 2: Launch Gradio demo
```bash
apt update && apt install ffmpeg -y # for demo
...
### Usage 3: Inference from files directly
```bash
